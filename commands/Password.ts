...
export function generatePassword(length: number, useUpper: boolean, useLower: boolean, useNumbers: boolean, useSymbols: boolean): string {
  ...
  if (!validChars) {
    throw new Error('You must select at least one option to create a password!');
  }
